...
This is the same math already used (and presumably validated in the source paper, Dikici et al. 2024) by `On-Track-SysID/src/on_track_sys_id.py`'s `publish_estimates()` / `generate_predictions.py` — `tire_force_benchmark` re-implements it independently (rather than subscribing to that node's output) so it can benchmark the nominal model's state-prediction accuracy standalone, using the real inter-sample `dt` from odometry timestamps and the same guards against a non-monotonic timestamp, an untrustworthy timing gap (`dt > 0.2s`), or a vehicle slower than `state_min_speed_mps` (default `1.0`). That last gate is stricter than `on_track_sys_id.py`'s own `v_x < 0.1`, deliberately: slip angle is `atan(v_y/v_x)`, so a few tenths of a m/s saturates the tire curve and predicts yaw rates of several rad/s against a ground truth of a few tenths - the low-speed bootstrap phase then dominates the state metrics. `1.0` is the same threshold `collect_data()` uses to decide a sample is worth identifying on.
...
- `on_track_sys_id` has its own forward of the same shape (also `benchmark_update_params_enable`, see `On-Track-SysID/launch/sys_id.launch.py`), used when the benchmark runs alongside the identification node without the manager stack in the loop. It fires from `handle_pending_ack()`, i.e. only once `adaptive_controller_manager` has **acked** that exact param set — not from `run_identification_cycle()` where it used to fire. Both timings put the same requirement on the benchmark: it starts collecting on a model the car is actually driving on, and never scores an identification the manager rejected, nor the interval between training finishing and the ack landing. (No manager means no identification cycle at all — `run_identification_cycle()` returns early while `sysid/update_params` is unavailable — so this cannot strand the benchmark waiting.)
