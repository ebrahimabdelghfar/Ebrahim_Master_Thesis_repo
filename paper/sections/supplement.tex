...
only through the slip angles of (3): the tire law (4) is
...
Equation~(3) composes a division by $v_{x}$ with an arctangent; a
...
Appending $\alpha_{f},\alpha_{r}$, computed from (3) with the
\emph{measured} geometry of Table~I of the main paper, hands the network the two
arguments of (4) directly. Three properties motivated the choice.
...
(1)--(2), with the axle forces re-evaluated through
(4) at the corrected slip angles, and penalises the squared lateral
...
(1)--(2) neglect load transfer and transient tire
...
The nominal-model half of \eqref{sup:pinnloss}---the nominal rollout, the
mirrored inputs and the constant terms of the symmetry residual---depends only
on quantities fixed for the whole training call and is precomputed once (see
...
of (3) and cut into $W=20$ sample windows. (b) The residual stack:
...
decimal places (Table~\ref{sup:runtime}). We deliberately did \emph{not} touch
...
on the hardware a vehicle carries; Table~\ref{sup:runtime} is the measurement,
...
Table~\ref{sup:baseline} places the pipeline as published~\cite{ontrack2025}
...
symmetry and input-gradient penalties, \eqref{sup:pinnloss}\\
...
$\rightarrow$ \SI{20.9}{\second}, exact (Table~\ref{sup:runtime})\\
