...
listed so that a reader can judge how far each result depends on them, and so
that each can be discharged by a specific measurement.

\subsection*{A1. Yaw inertia $I_{z}$}
$I_{z}=\SI{51.1}{\kilo\gram\meter\squared}$ is carried in the model file and
is \emph{not} measured. It enters \eqref{eq:omdot} directly, so it scales the
yaw-rate residual the network is trained on, and it enters the
stiffness-adaptive sub-stepping of Section~\ref{subsec:gates} through
$(l_{f}^{2}C_{f}+l_{r}^{2}C_{r})/I_{z}$. \emph{How to discharge:} apply a known
yaw moment (a step steer at constant speed, axle forces read from the force
feedback topic) and fit $I_{z}$ from the measured $\dot\omega$; or read the
value the physics engine uses from the actor's inertia tensor.
...
than the geometry does (51.9/48.1), a \SI{1.4}{\percent} disagreement,
unresolved and below the level at which any conclusion here would change.

\subsection*{A3. CG height and load transfer}
$h_{\mathrm{cg}}=\SI{0.3}{\meter}$ is carried in the model file and unused:
the single-track model neglects load transfer, following the baseline, and
normal loads are static. On a full-scale vehicle at \SI{1}{\g} this is a
stronger approximation than at 1:10, and a candidate explanation for part of
the residual the network absorbs.

\subsection*{A4. Steering-angle sign and units}
\texttt{feedback/\allowbreak steering\_angle} is published in degrees and treated as
...
single-track $\delta$ being this single reported value.

\subsection*{A5. Steady-state force recovery}
Equation~\eqref{eq:ssforces} is not a force measurement. Being fully determined
by $v_{x}\omega$ it misattributes transients, and it is applied only to the
\emph{synthetic}, quasi-steady rollout, where the steady-state assumption is
constructed rather than hoped for. Its adequacy was nonetheless checked on
measured data against the simulator's own axle force (correlation \num{0.993},
slope \num{0.94}); we treat that as validating the relation, not a steady-state
assumption on recorded laps.
...
choice of $1.5$ is empirical, sufficient to reach the tire's peak at the
corrected rollout speed while removing the observed drift; it is not derived.
...
since ROS~2 Humble ships CPython~3.10 ABI extensions only), PyTorch with CUDA
on an RTX~2080-class GPU. The bridge is built and sourced as a separate overlay
on that workspace, so its message packages
...
epochs, full-batch Adam at $5\times10^{-4}$, and the one-step model is
discretised at $T_{s}=\SI{0.035}{\second}$ (Section~\ref{subsec:data}).
...
client-side steering PID is present, the commanded road-wheel angle passing
...
physics the vehicle is driven by, which is what allows an on-track identifier
to be scored against the forces the plant generated. Four measured properties
bound what may be read off it. (i)~Only \texttt{lateral\_force} is force ground
truth; it is the channel every validation in this paper uses.
...
and \texttt{wheel\_torque} is an exact identity of it, so neither carries
...
sign change across the boundary, verified rather than assumed.
...
REP-103~\cite{rep103}, right-handed ENU for the world and forward-left-up for
the body, and the identification equations are written in the body frame with
$\alpha$ and $F_{y}$ positive to the left, the SAE-style axis
convention~\cite{saej670,pacejka2012book}. The bridge is a per-channel
conversion rather than a single transform: position and yaw flip sign, twist is
additionally rotated into the body frame by $q^{-1}$, the IMU's $a_{y}$ flips,
the steering \emph{command} flips on the way out, and the tire telemetry's slip
angle does \emph{not}, because the frame flip and CARLA's own sign convention
for \texttt{lat\_slip} cancel. That last is the trap: a sign error there is
invisible in the states and inverts the identified curve, so it was checked
against a slip angle rebuilt from the chassis state in a steady
\SI{4}{\meter\per\second} turn at \SI{0.20}{\radian}, equal in magnitude to
within \SI{2}{\percent} front and \SI{15}{\percent} rear.
