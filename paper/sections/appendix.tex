...
forces read from \texttt{feedback/\allowbreak tire\_forces}) and fit $I_{z}$ from the
...
hardware). All identification data is stamped from the simulation clock. We
assume the physics step is uniform and that dead-reckoned odometry samples
(disabled here via \texttt{follow\_server\_rate}) do not enter the
...
(\texttt{sim\_manager\_\allowbreak msgs}) resolve for the ground-truth topics;
CARLA server 0.9.16 / Unreal Engine 4.26; \texttt{acados} 0.5.5 with
partial-condensing HPIPM. Identification runs at \SI{50}{\hertz} with a
\SI{30}{\second} buffer, six co-identification iterations, 200 training
epochs, full-batch Adam at $5\times10^{-4}$.

\subsection*{A9. Regression tests}
Eight tests pin the identifiability signature (no fitted coefficient on a
bound; $l_{wb}\equiv l_{f}+l_{r}$; the rollout speed derivation) and run under
\texttt{colcon test}, so that the degenerate-fit signature cannot return
silently. This is a reproducibility mechanism rather than a scientific
result, but it is the mechanism that would catch a regression of the paper's
central finding.
