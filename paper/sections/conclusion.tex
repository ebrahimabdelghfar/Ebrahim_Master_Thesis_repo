...
configuration and not the tire. A rollout speed appropriate at 1:10 caps
...
optimiser resolves the ambiguity on a box edge. Two further properties of the
loop follow from the same analysis. The fit's residual depends on $B$, $C$ and
$D$ only through their product, because stage~3 reconstructs its forces from
the rollout's own quasi-steady yaw balance and so observes nothing but the
lateral acceleration the previous coefficient set produced; the peak factor is
therefore not recoverable from the rollout at any excitation and must be
measured on the recorded trace and held. And because each fit is handed the
trajectory its own previous answer generated, the loop is a positive feedback
path rather than a contraction, so stage~4 is reformulated as a relaxed
...
configured prior was measured on so that returning the prior scores badly, the
corrected pipeline recovers axle cornering stiffness to \SI{3.2}{\percent} and
\SI{1.4}{\percent} and the peak factor to \SI{11}{\percent}, against an
inherited configuration that rails $D$ on its \num{2.0} box constraint in every
cycle and overstates stiffness by \SIrange{118}{132}{\percent}; axle force RMSE
is over ten times lower and MPC lateral tracking error \SI{32}{\percent} lower,
at unchanged solver cost. Those numbers are an identification and not a
preserved prior, with three qualifications: the peak factor is identified by
the brush estimator on the recorded trace, not by the Pacejka fit, which cannot
...
estimates with a confidence interval. That the peak factor is recoverable from
the rollout \emph{when the excitation reaches it} is shown on a known tire in
Section~\ref{subsec:sweep}, not on these laps. We further argue that
...
\SI{1.0}{\g}, was regenerated because that gate refused it rather than loosened
for it.
...
ablations are offline replays, enough to attribute the result but not to
confirm the ordering survives closed-loop coupling. Each is a scenario-file
row.
\item Sweep the friction rather than merely mismatching it. These runs put the
prior \SI{44}{\percent} away from a \emph{static} surface, showing the pipeline
moves on evidence but not that it tracks; the step and decay schedules already
in the scenario file are the intended vehicle, and the strongest available
argument for identifying on track at all.
\item Excite the peak. The lap reaches
$P_{99}(|\alpha|)=\SI{2.7}{\degree}$ and \SI{5.8}{\degree} at most, against a
plant whose curve peaks at \SI{8.0}{\degree} on this surface, at an axle
utilisation of \num{0.71}. That is enough for the friction gate to release, and
it is also where the brush estimator's \SI{11}{\percent} bias comes from; a
trajectory or injected manoeuvre traversing the peak would remove it.
...
the IMU-versus-kinematics cross-check guarding the mass and inertia
assumptions. Since the published peak factor now \emph{is} that estimate, its
bias is the pipeline's bias.
\item Report lap time on the simulator's own clock, which currently makes lap
time unusable as a comparison metric
...
identification toward active, safety-bounded experiment design: optimal input
design~\cite{goodwin1977design} under a safety constraint, where the literature
