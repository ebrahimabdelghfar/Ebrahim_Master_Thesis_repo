...
only as a floor. Together these produce identifications with no railed
coefficient and a peak factor of \num{1.009}/\num{1.002} against a plant
realising \num{1.05}, with the correct understeer sign. Run against the
inherited configuration over three timed laps on a static surface, the
corrected pipeline \emph{preserves} that peak factor to \num{0.041} and
...
Preserves, not identifies: this trajectory never approaches the tire's peak,
the accepted fits return the configured prior to four decimal places, and the
reported peak-friction accuracy is therefore the prior's
...
confidence interval. The inherited configuration is better on front-axle force
and on lateral-velocity prediction, because the lap reaches \SI{2.2}{\degree}
of slip and a wrong peak factor is not observable there, which is the same
identifiability statement seen from the metric side. We further argue that
