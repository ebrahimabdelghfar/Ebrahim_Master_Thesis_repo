...
# 2. UPDATE PATHS TO YOUR TRAINED MODEL
param_file = "../cfgs/model/deep_dynamics.yaml"
# Ensure this points to your valid .pth file
state_dict = "../output/deep_dynamics/ddm_sim_repro_v3/epoch_388.pth"

print(f"Loading config from: {param_file}")
...
ddm.to(device) 
ddm.load_state_dict(torch.load(state_dict, map_location=device))

scaler_path = os.path.join(os.path.dirname(state_dict), "scaler.pkl")
...
# Manual init for data_x
data_x = np.concatenate([x_init, np.zeros(2)]) 
...
# planner based on BayesOpt
	...
	if idt > 15:
		...
		ddm_data_norm = torch.from_numpy(np.expand_dims(ddm_scaler.transform(ddm_data), axis=0)).float().to(device)
		ddm_data = torch.from_numpy(np.expand_dims(ddm_data, axis=0)).float().to(device)
		if ddm.is_rnn:
	...
	# FIX: Unpack tuple return from sim_continuous
	sim_out = model.sim_continuous(states[:,idt], inputs[:,idt].reshape(-1,1), [0, Ts])
	x_next = sim_out[0] # Extract the state trajectory
	
	# Construct data_x manually (Next State + Current Input)
	data_x = np.concatenate([x_next[:,-1], inputs[:,idt]])
	...
	if idt > 15:
		...
		for idh in range(horizon):
			# FIX: Unpack tuple return in prediction loop too
			sim_out_mpc = dpm_model.sim_continuous(hstates[:,idh], umpc[:,idh].reshape(-1,1), [0, Ts])
			x_next_mpc = sim_out_mpc[0]
			
			hstates[:,idh+1] = x_next_mpc[:,-1]
		...
		# FIX: Wrap single point in brackets []
