...
\begin{figure*}[t]
\centering
\resizebox{0.84\textwidth}{!}{\input{figures/pipeline}}
...
\eqref{eq:pinnloss} gives $\lambda_{s}$. This is a correction to the objective
as published, not an implementation detail.
...
Two changes close the loop. First, an \textbf{extrapolation bound}: the sweep
...
(Section~\ref{subsec:assumptions}, A6). Second, a \textbf{frozen prior}: the
...
carry information; they have to be damped for it to survive.
...
its asymptote: writing $r=|F_{y}|/(C_{\alpha}|\tan\alpha|)$ for the force's
...
the \emph{accepted} model only. \textbf{One-step prediction} on a held-out run,
...
\SI{1}{\meter\per\second}. \textbf{Direct force comparison} against
...
tire. \textbf{Closed-loop tracking} under the MPC: lateral deviation, its
