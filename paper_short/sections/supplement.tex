...
$l_{wb}\equiv l_{f}+l_{r}$ is enforced (Section~V-F of the main paper): an
...
before squaring, as Section~V-A1 of the main paper derives. Without that
...
without altering the arithmetic. None is novel as a technique. What they buy
here is a pipeline that is real-time-plausible on commodity hardware, with the
...
Section~V-G of the main paper, so the temporal residual is a deployable
...
the rollout \emph{speed} of Section~V-B of the main paper is taken from the
measured lap in both runs of Section~VI-C of the main paper, so that section
measures the remaining eight settings and not the rollout speed.
...
simulator reports per wheel ($\mu=0.70$, static for both runs). The corrected
...
bound for the entire run rather than from a mis-estimate.}
\label{fig:muerror}
\end{figure}
...
slope through (5) of the main paper.}
\label{fig:forcebars}
\end{figure}
