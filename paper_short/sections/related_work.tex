...
the manoeuvre identifies, and pinned for the co-identification loop --- a
...
\emph{insufficient}: the degenerate fit that motivated this work satisfied every
...
of~\cite{ontrack2025} is the pipeline we extend; its key idea, to query the
learned model only where it interpolates and extract physically parameterised
coefficients from it, is retained unchanged, and what we correct is the
mechanism deciding \emph{where} the learned model is queried.
