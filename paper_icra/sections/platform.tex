...
rather than against a second model; that channel never enters the
identification loop. And the server runs synchronously at a fixed
...
that peak, so a Magic Formula can match it up to the peak and not beyond; its
scale is set by $\mu$ and $C_{\mathrm{lat}}$, both published by the simulator,
so the plant's curve is read off rather than fitted: $\mu=\num{1.05}$ effective (a configured
wheel friction of \num{1.5} times the road surface's \num{0.70}, with a
realised peak axle friction of \numrange{1.00}{1.05}), $k_{y}=\SI{17}{\per
...
pure-pursuit controller, which needs no tire knowledge---only the
wheelbase---and is therefore a valid bootstrap, as in the
baseline~\cite{ontrack2025}. Odometry, the achieved steering angle and the
drive command are sampled into a ring buffer for \SI{30}{\second}, gated on
$v_{x}>\SI{1}{\meter\per\second}$, matching the baseline's data budget;
re-identification retriggers every \SI{30}{\second}. The model is
discretised at $T_{s}=\SI{0.035}{\second}$, a setting carried because the
identification proved insensitive to it: identifying at \SI{0.020}{\second}
returns the same last accepted coefficient set to four decimal places in the
corrected configuration, and leaves $D$ on its \num{2.0} bound in the inherited
one, so the box-edge signature below is not an artefact of the step mismatch.
