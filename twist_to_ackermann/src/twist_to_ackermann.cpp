...
: Node("twist_to_ackermann", options)
{
  ...
  _wheelbase = this->declare_parameter("wheelbase", 1.57);
  auto twist_topic = this->declare_parameter("twist_topic", "/cmd_vel");
  auto ack_topic = this->declare_parameter("ackermann_topic", "/ack_vel");

  if (_use_stamps) {
    _twists_sub = this->create_subscription<geometry_msgs::msg::TwistStamped>(
      twist_topic, 10, std::bind(&TwistToAckermann::twists_cb, this, std::placeholders::_1));
    _acks_pub = this->create_publisher<ackermann_msgs::msg::AckermannDriveStamped>(ack_topic, 10);
  } else {
    _twist_sub = this->create_subscription<geometry_msgs::msg::Twist>(
      twist_topic, 10, std::bind(&TwistToAckermann::twist_cb, this, std::placeholders::_1));

    _ack_pub = this->create_publisher<ackermann_msgs::msg::AckermannDriveStamped>(ack_topic, 10);
  }
}
